...
import { LINE_ITEMS, LINE_ITEM_DISCOUNT_COMMISSION, propTypes } from '../../util/types';
...
const LineItemUnknownItemsMaybe = props => {
  ...
  const nonDiscountLineItems = items.filter(
    item => item.code !== LINE_ITEM_DISCOUNT_COMMISSION
  );
  return nonDiscountLineItems.length > 0 ? (
    <React.Fragment>
      {nonDiscountLineItems.map((item, i) => {
